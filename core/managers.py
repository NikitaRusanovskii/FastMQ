...
class FiltersManager(IFiltersManager):
    ...
    async def subscribe_on(self, name: str, websocket_id: int):
        ...
        logger.info((f"Client with ID {websocket_id} " f"subscribed on filter {name}."))


class Registry(IRegistry):
    def __init__(self, start_consumer_id, start_producer_id):
        self.prod_ids = {}
        self.cons_ids = {}
        self.sock_ids = {}
    ...
    async def get_consumer_by_id(self, id: int | None) -> list[int] | None:
        if id is None:
            return None
        async with self.lock:
            try:
    ...
    async def get_id_by_websocket(self, websocket: ClientConnection) -> int | None:
        async with self.lock:
            try:
...
class ClientFabric(IClientFabric):
    def __init__(self, registry: IRegistry):
        ...
        self.roles = {"consumer": Consumer, "producer": Producer}
        self.regs = {
            "consumer": self.registry.add_consumer,
            "producer": self.registry.add_producer,
        }

    async def create(self, websocket: websockets.ClientConnection, path: str) -> Unit:
        role = path.strip("/")
