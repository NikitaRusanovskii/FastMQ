...
from .managers import ClientFabric, FiltersManager, Registry
...
async def main():
    ...
    fm = FiltersManager()
    rg = Registry(
        start_consumer_id=start_consumer_id,
    ...
    await rb.initialize()
    mq = MessageQueue(rb, rg, attempts_count, time_sleep, queue_pause, timeout)
    ...
    try:
        # await asyncio.gather(server_task, cli_task, message_queue_tast)
        await asyncio.gather(server_task, message_queue_tast)
    finally:
